...
func (b *Txm[CHAIN_ID, HEAD, ADDR, TX_HASH, BLOCK_HASH, R, SEQ, FEE]) runLoop() {
	...
	execReset := func(r *reset) {
		...
		if err := b.broadcaster.closeInternal(); err != nil {
			b.lggr.Panicw(fmt.Sprintf("Failed to Close Broadcaster: %v", err), "err", err)
		}
		if err := b.confirmer.closeInternal(); err != nil {
			b.lggr.Panicw(fmt.Sprintf("Failed to Close Confirmer: %v", err), "err", err)
		}
		if r != nil {
			r.f()
			close(r.done)
			...
			for {
				...
				case <-time.After(backoff.Duration()):
					if err := b.broadcaster.startInternal(ctx); err != nil {
						logger.Criticalw(b.lggr, "Failed to start Broadcaster", "err", err)
			...
			for {
				...
				case <-time.After(backoff.Duration()):
					if err := b.confirmer.startInternal(); err != nil {
						logger.Criticalw(b.lggr, "Failed to start Confirmer", "err", err)
						b.SvcErrBuffer.Append(err)
						continue
					}
					return
	...
	for {
		...
		case <-b.chStop:
			...
			if err != nil && (!errors.Is(err, services.ErrAlreadyStopped) || !errors.Is(err, services.ErrCannotStopUnstarted)) {
				b.lggr.Errorw(fmt.Sprintf("Failed to Close Broadcaster: %v", err), "err", err)
			}
			err = b.confirmer.Close()
			if err != nil && (!errors.Is(err, services.ErrAlreadyStopped) || !errors.Is(err, services.ErrCannotStopUnstarted)) {
				b.lggr.Errorw(fmt.Sprintf("Failed to Close Confirmer: %v", err), "err", err)
			}
			err = b.tracker.Close()
			if err != nil && (!errors.Is(err, services.ErrAlreadyStopped) || !errors.Is(err, services.ErrCannotStopUnstarted)) {
				b.lggr.Errorw(fmt.Sprintf("Failed to Close Tracker: %v", err), "err", err)
			}
			return
		case <-keysChanged:
			// This check prevents the weird edge-case where you can select
			...
			if err != nil {
				logger.Criticalf(b.lggr, "Failed to reload key states after key change")
				b.SvcErrBuffer.Append(err)
				continue
			}
			b.lggr.Debugw("Keys changed, reloading", "enabledAddresses", enabledAddresses)
...
func (b *Txm[CHAIN_ID, HEAD, ADDR, TX_HASH, BLOCK_HASH, R, SEQ, FEE]) OnNewLongestChain(ctx context.Context, head HEAD) {
	ok := b.IfStarted(func() {
		...
		case b.chHeads <- head:
		case <-ctx.Done():
			b.lggr.Errorw("Timed out handling head", "blockNum", head.BlockNumber(), "ctxErr", ctx.Err())
		}
	})
	if !ok {
		b.lggr.Debugw("Not started; ignoring head", "head", head, "state", b.State())
	}
}
...
func (b *Txm[CHAIN_ID, HEAD, ADDR, TX_HASH, BLOCK_HASH, R, SEQ, FEE]) CreateTransaction(ctx context.Context, txRequest txmgrtypes.TxRequest[ADDR, TX_HASH]) (tx txmgrtypes.Tx[CHAIN_ID, ADDR, TX_HASH, BLOCK_HASH, SEQ, FEE], err error) {
	...
	if b.txConfig.ForwardersEnabled() && (!utils.IsZero(txRequest.ForwarderAddress)) {
		...
		if fwdErr == nil {
			...
			b.lggr.Errorf("Failed to use forwarder set upstream: %w", fwdErr.Error())
		}
	}
